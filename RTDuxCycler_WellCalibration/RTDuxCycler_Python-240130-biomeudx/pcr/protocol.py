...
default_protocol = Protocol('Default', [
    {'Label' : 1, 'Temp' : 60, 'Time' : 5, 'Shot' : False}, 
    {'Label' : 2, 'Temp' : 90, 'Time' : 5, 'Shot' : False}, 
    {'Label' : 3, 'Temp' : 65, 'Time' : 5, 'Shot' : True},
    {'Label' :'GOTO','Temp' : 2,'Time' : 2, 'Shot' : False},
    {'Label' : 4, 'Temp' : 40, 'Time' : 5, 'Shot' : False}
])
...
def check_protocol(protocol):
    ...
    for line in protocol:
        ...
        if label == 'GOTO':     # GOTO action check
            ...
            if time < 5:
                raise PCRProtocolError('Invalid protocol time : protocol time must be at least 5 seconds. line {line_number}')
        ...
        actions.append({'Label' : label, 'Temp' : temp, 'Time' : time, 'Shot' : False})
        print(actions)
    return actions
