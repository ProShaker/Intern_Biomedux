...
class PCRTask:
    ...
    def check_status(self):
        ...
        if self.command == Command.NOP:
            ...
            if state_oper == StateOper.RUN_SHOOT:
                pass
            elif state_oper != StateOper.INIT:
            ...
            if self.optic.shot_thread.shot_end:
                ...
                if RxAction.rx_buffer["Current_Operation"] == StateOper.RUN_SHOOT:
                    print("hiihoi")
                    self.command = Command.RESUME
                else:
                    self.command = Command.NOP
            ...
            if  self.action_num != (len(self.protocol.actions) - 1):
                self.action_num += 1
            else:
                print("send task end")
                self.command = Command.TASK_END
    ...
    def calc_temp(self):
        ...
        total_time = RxAction.rx_buffer['Total_TimeLeft']
        current_action = RxAction.rx_buffer['Current_Action']

        self.mainUI.frame_ctrl.set_temp(self.chamber_temp, self.heater_temp)
        self.mainUI.frame_ctrl.set_total_time(total_time)
        self.mainUI.frame_ctrl.set_cur_label(current_action)
        
        # self.mainUI.frame_ctrl.time(self.)
        # self.mainUI

    def line_task(self):
        if self.state == State.RUN:
            loop, _, left_time = RxAction.rx_buffer["Current_Loop"], RxAction.rx_buffer["Current_Action"], RxAction.rx_buffer["Sec_TimeLeft"]
            self.mainUI.frame_ctrl.set_loop((0 if loop == 255 else self.cycle_num-loop))
    ...
    def check_shot(self):
        ...
        if (self.running
            and RxAction.rx_buffer["Current_Operation"] == StateOper.RUN_SHOOT
            ...
            if self.cur_label in self.protocol.shot_labels: # If current label is shot label
                ...
                if (not self.optic.shot_thread.shotting
                    and target_temp-0.3 <= self.chamber_temp <= target_temp+0.3
                    and self.sec_TimeLeft <= 0):
